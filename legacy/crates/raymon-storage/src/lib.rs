...
use raymon_core::{FilterError as CoreFilterError, Filters as CoreFilters};
...
impl EntryFilter {
    fn matches(&self, meta: &OffsetMeta) -> bool {
...
mod tests {
    ...
    use raymon_core::{Entry as CoreEntry, Filters as CoreFilters, Origin, Payload as CorePayload, Screen};
    ...
    fn list_entries_preserves_insertion_order() {
        ...
        let listed = storage.list_entries(None);
        let ids: Vec<String> = listed.into_iter().map(|meta| meta.id).collect();
        assert_eq!(
            ids,
            vec![
                "entry-1".to_string(),
    ...
    fn list_entries_core_filters_match() {
        ...
        assert_eq!(listed.len(), 1);
        assert_eq!(listed[0].id, "entry-core");
    }
