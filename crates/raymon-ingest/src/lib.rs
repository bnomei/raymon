...
use raymon_core::{Entry, Event, RayEnvelope};
    ...
    C: Fn() -> u64,
{
    ...
    pub fn handle_inner(&self, body: &[u8]) -> Result<Entry, IngestError> {
        let value: Value = serde_json::from_slice(body).map_err(IngestError::InvalidJson)?;
        let envelope: RayEnvelope = serde_json::from_value(value)
            .map_err(|err| IngestError::InvalidEnvelope(err.to_string()))?;
